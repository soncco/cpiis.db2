...
public class Pagina implements Iterable<Tupla> {
    ...
    public void adicionarTuplaOrdenada(Tupla tupla) throws ArrayIndexOutOfBoundsException {
        if(hayEspacio(tupla)) {
            int id = tupla.getIdTupla().getNumero();
            boolean flag = true;
            tuplas.add(tupla);
            int i = tuplas.size()-1;
            boolean flag2 = false;
            while((i>=0) && flag) {
                if (i>0) {
                    if(tuplas.get(i-1).getIdTupla().getNumero() > id) {
                        tuplas.remove(tupla);
                        tuplas.add(i-1, tupla);
                        i--;
                    } else {
                        flag = false;
                    }
